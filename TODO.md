...
- [x] Patch the GeoShield optimization loop to directly include Attack-VLLM augmentations and visual contrastive/relative proxy losses.
- [x] Add DINOv2 and open-VLLM surrogate adapters through the adaptive external overlay.
- [x] Add SAM box-refinement helper after GroundingDINO detections.
...
- [ ] Validate a one-image adaptive GPU run with CLIP+DINOv2.
- [ ] Validate Qwen2VL and LLaVANeXT surrogates only after confirming GPU memory/checkpoint availability.
